...
Carbon Tracker operates not only as a standalone web application but also as a
**production-grade backend API service** for external platforms. Third-party
applications can consume our typed, versioned endpoints to power their own
sustainability features — turning Carbon Tracker into shared climate
infrastructure.

**Nyx Browser** — our first official integration partner — is an eco-friendly web
browser optimised for sustainable computing. Nyx consumes our dedicated API
endpoint to power its native **Climate & Sustainability Dashboard**, giving
users real-time visibility into the environmental impact of their browsing
sessions.

| Integration | Endpoint | Status |
| --- | --- | --- |
| **Nyx Browser** | `GET /api/v1/nyx/metrics` | ✅ Live |
...
> Interested in integrating? The full OpenAPI schema is available at
> [`/docs`](https://carbon-tracker-gulb.onrender.com/docs).
