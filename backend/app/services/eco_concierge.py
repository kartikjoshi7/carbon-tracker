...
def generate_insights_sync(category: str, metric_value: float, calculated_co2: float) -> str:
    ...
    targeting SVIT Vasad / Dabhoi PG persona.
    """
    ...
    try:
        ...
        prompt = f"""
        You are an Eco-Concierge tailored to an IT engineering student at SVIT Vasad living in a shared PG in Dabhoi.
        ...
        Focus on split bills, shared rickshaws from Dabhoi to SVIT, and cafeteria waste if applicable.
        """
...
def _fallback_generator(category: str, metric_value: float, calculated_co2: float) -> str:
    if category == "energy":
        return f"Talk to your roommates to optimize the split AC usage. Saving {calculated_co2}kg CO2e lowers the Dabhoi PG electricity bill for everyone."
    elif category == "transit":
        return f"Coordinating a shared rickshaw from Dabhoi to SVIT campus reduces your per-capita emission by {calculated_co2}kg CO2e compared to riding alone!"
    elif category == "waste":
        return f"SVIT cafeteria portions are large. Saving {metric_value}g of food lowers waste footprint by {calculated_co2}kg CO2e."
